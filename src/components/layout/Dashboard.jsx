...
function StatCard({ label, value, colour }) {
  ...
  return (
    <div style={{ ...dashboardCardStyle, padding: '18px 20px', flex: 1, minWidth: 0 }}>
...
export default function Dashboard({ onNavigate, onOpenProject }) {
  ...
  // Load invoices directly from Supabase — not from app state
  ...
  const refreshBriefing = useCallback(async () => {
    ...
    setBriefing('');
    try {
      const projSummary = projects.filter(p => p.status !== 'complete').slice(0, 10)
        .map(p => `${p.ref}: ${p.address}, status: ${p.status}`).join('\n');
      const deadlineSummary = upcoming.slice(0, 5)
        .map(u => `${u.label} on ${new Date(u.date).toLocaleDateString('en-GB')}`).join('\n');
  ...
  return (
    <div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 16, background: '#f1f3f6', minHeight: '100%' }}>
      ...
      {/* ── Row 2: Morning briefing + Cash flow ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr', gap: 14 }}>
        ...
        <div style={{ ...dashboardCardStyle, padding: '16px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <button onClick={refreshBriefing} disabled={briefingLoading}
              style={{ background: 'none', border: 'none', color: 'var(--blue)', fontSize: 12, cursor: 'pointer', fontWeight: 500, padding: 0 }}>
        ...
        {/* Cash flow — now uses real invoice data */}
        <div style={{ ...dashboardCardStyle, padding: '16px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.6px' }}>
              💰 Cash Flow
            </div>
            <span onClick={() => onNavigate('accounting')}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0 0', fontSize: 13 }}>
            ...
            <span style={{ fontWeight: 700, color: 'var(--blue)', fontSize: 15 }}>{fmt(feePipeline)}</span>
          </div>
        </div>
      </div>

      {/* ── Row 3: Leads + Next 14 days ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 14 }}>
        ...
        <div style={{ ...dashboardCardStyle, padding: '16px 20px' }}>
          ...
          {activeLeads.length === 0 ? (
            ...
            return (
              <div key={lead.id} onClick={() => onNavigate('leads')}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 10px', borderRadius: 12, marginBottom: 6, border: '1px solid #e7eaf0', cursor: 'pointer', background: '#fafafa' }}
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: ageColour, background: `${ageColour}22`, padding: '2px 7px', borderRadius: 99 }}>{ageDays}d</span>
                  <span style={{ fontSize: 11, background: 'var(--blue-bg)', color: 'var(--blue)', padding: '2px 6px', borderRadius: 99, fontWeight: 600 }}>AI</span>
                </div>
              </div>
            );
          })}
        ...
        {/* Next 14 days */}
        <div style={{ ...dashboardCardStyle, padding: '16px 20px', overflowY: 'auto', maxHeight: 340 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <div key={label} style={{ marginBottom: 12 }}>
              ...
              {items.map((item, i) => (
                <div key={i}
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', fontSize: 12.5, color: 'var(--text2)', cursor: item.projectId ? 'pointer' : 'default', border: '1px solid #edf0f4', background: '#fafafa', borderRadius: 10, marginBottom: 5 }}
                ...
                >
                  ...
                  {item.ref && <span style={{ fontSize: 10.5, color: 'var(--text3)', flexShrink: 0 }}>{item.ref}</span>}

...
export default function AwardReview() {
  ...
  const handleReview = async () => {
    ...
    try {
      ...
      let prompt;
      if (isBenchmarkMode) {
        prompt = `Please review this party wall award against my master benchmark template.

Award to review: [attached as document]

${AWARD_REVIEW_SYSTEM_PROMPT}`;
      } else {
        prompt = `Please compare these two party wall award drafts.

Document 1 is the base draft. Document 2 is a revised or alternative version.
...
Be specific — quote the exact wording differences. Reference the Act where relevant.`;
      ...
      // Call the Anthropic API directly with document support
      const messages = [
        {
          role: 'user',
          content: [
            {
              ...
              title: isBenchmarkMode ? 'Award to Review' : 'Document 1 — Base Draft',
            },
            ...(base64_2 ? [{
              type: 'document',
              source: {
              ...
              title: 'Document 2 — Revised Draft',
            }] : []),
      ...
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 4000,
          system: `You are Ely, a party wall surveying expert assistant to Itzik Darel MIPWS ACIArb of Square One Consulting. You have deep knowledge of the Party Wall etc. Act 1996, award drafting, and best practice. You review awards with the eye of an experienced practitioner — precise, direct, and focused on what actually matters legally and practically.`,
          messages,
        }),
      });

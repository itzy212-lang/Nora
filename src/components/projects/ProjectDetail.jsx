...
// ── AO Card ───────────────────────────────────────────────────────────────────
function AOCard({ ao, onOpenComposer, onGenerateAOLOA, loaLoading }) {
      ...
      <div style={{ display: 'flex' }}>
        ...
        <div style={{ flex: 1, padding: '14px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 10 }}>
            ...
            >
              {loaLoading ? 'Generating…' : 'Agreed Surveyor LoA'}
            </button>
          </div>
        </div>
      </div>
...
export default function ProjectDetail({ project, onBack, onOpenComposer, onRaiseInvoice, onOpenSOC }) {
  ...
  const handleGenerateBOLOA = useCallback(async () => {
    ...
    try {
      ...
      if (!result.success) {
        alert(result.error || 'Could not generate BO LOA.');
      }
    } catch (err) {
      alert(err.message || 'Could not generate BO LOA.');
    } finally {
      setLoaLoading(null);
    }
  ...
  const handleGenerateAOLOA = useCallback(async (ao) => {
    setLoaLoading(`ao-${ao.id || ao.num || ao.name || 'unknown'}`);
    ...
    try {
      ...
      if (!result.success) {
        alert(result.error || 'Could not generate AO LOA.');
      }
    } catch (err) {
      alert(err.message || 'Could not generate AO LOA.');
    } finally {
      setLoaLoading(null);
    }
  }, [generateDocument, project]);
            ...
            <div style={{ ...card({ padding: '18px 20px', marginBottom: 16 }) }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 24px', marginBottom: 14 }}>
                ...
                <div>
                  ...
                  >
                    {loaLoading === 'bo' ? 'Generating…' : '📄 Send BO LoA'}
                  </button>
                </div>

...
STEP 1 — SITE ADDRESS
Read the site address ONLY from the drawing's title block — the bordered information panel (almost always bottom-right or bottom corner of the sheet) containing the client name, project address, drawing number, scale and revision. Do NOT use house numbers or labels shown on a floor plan itself (e.g. "No 20", "No 22", "No 24" used only to identify neighbouring/adjoining properties for context on the plan) — those are never the site address, even if they appear larger or more prominent than the title block. The full address MUST include the postcode if it is printed anywhere in the title block — do not truncate the address at the town/county and omit the postcode; check the title block carefully for it, as it is often on its own line after the town.
...
STEP 3 — SCOPE ITEMS FOCUS ON WHAT IS CHANGING
...
EVERY DISTINCT ROOM NEEDS ITS OWN ITEM(S) — do not let one room become a passing mention inside a broader item about a different room or a general area. If the proposed plan shows a Study, En-suite, or any other named space undergoing work, that room must have at least one scope item genuinely zoned to it — not just be named in a sentence inside "Ground floor internal reconfiguration" or similar general items while its own actual scope of work (finishes, fittings, services) gets folded into other rooms. Before finishing, check every room name from the proposed plan appears as a zone value on at least one scope item — if a named room has been mentioned but has no scope item zoned to it, add one.

CONSOLIDATION APPLIES WITHIN A ROOM, NOT ACROSS ROOMS — avoid multiple fragmented items describing the same work in the same room from different angles (that is what "consolidate" means here). It does NOT mean merging genuinely different rooms into one broader item — each room still gets its own item(s) even if the same trade (e.g. plastering, electrics) is being consolidated within that room's own item.

STEP 4 — ZONE FOR EACH SCOPE ITEM — THIS IS A REQUIRED FIELD ON EVERY ITEM, NEVER OMIT IT
Every single scope item you output MUST have a "zone" value filled in — this is how rooms get created downstream, so a missing zone means that item is lost from the room breakdown entirely. Two zones are always fixed and used consistently regardless of which room the work is physically near:
...
Everything else uses the specific room/space name from the PROPOSED layout that the work is genuinely tied to (e.g. "Kitchen", "Bathroom", "Loft Room 1") — use the proposed label, never an existing-only room name that ceases to exist. This includes:
...
MULTIPLE ROOMS OF THE SAME TYPE — if the proposed layout shows more than one room of the same kind (e.g. three bedrooms, two loft rooms), you MUST distinguish them individually. Preference order: (1) use the drawing's own labels if present (e.g. "Bedroom 1", "Bedroom 2", "Bedroom 3"); (2) if the drawing shows no numbering or distinguishing annotation at all, number them yourself in the order you encounter them reading the floor plan (e.g. "Bedroom 1", "Bedroom 2", "Bedroom 3") — do this even without a positional cue, since sequential numbering is still far better than one shared zone; (3) only use a positional description (e.g. "Bedroom (Front)") if that is clearly more useful than a plain number. NEVER use a single generic zone like "Bedroom" for scope items that belong to different physical bedrooms — this incorrectly merges separate rooms into one, and is not acceptable even when the drawing itself does not number them. Check the full drawing set for how many distinct instances of a room type actually exist before assigning zones, and number/distinguish every one of them consistently across all scope items.
...
{
  "site_address": "from the title block only, including postcode if shown",
  ...
  "scope_items": [
    {
      ...
      "zone": "Structure / External / a specific proposed room name / Unallocated"
    }
  ]
}`,

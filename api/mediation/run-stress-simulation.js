...
const EVENT_TYPE = 'phoenix_stress_simulation_v1';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (existing?.event_data?.result) return res.status(200).json({ cached: true, ...existing.event_data.result });
    ...
    for (let i = 0; i < partyTurns.length; i++) {
      ...
      const mediatorText = result.text;
      transcript.push({ turn: i + 1, party: partyTurns[i], mediator: mediatorText });
      messages.push({ role: 'assistant', content: mediatorText });
    ...
    const { error: insertError } = await sb.from('mediation_audit_events').insert({ mediation_id: null, actor_type: 'system_test', actor_id: 'phoenix_regression', event_type: EVENT_TYPE, event_data: { result: payload } });

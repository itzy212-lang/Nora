...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Load firm settings (personality etc.) ────────────────────────────
    let noraPersonality = 'professional';
    try {
      const sb = getSupabase();
      if (sb) {
        const { data: firmData } = await sb.from('firm_settings').select('nora_personality').limit(1).maybeSingle();
        if (firmData?.nora_personality) noraPersonality = firmData.nora_personality;
      }
    } catch (_) {}

    const PERSONALITY_INSTRUCTIONS = {
      professional: `Write as Itzik would write on his best day: direct, professionally confident, conversational between peers, and commercially precise.\n\nPreserve his directness, the rhetorical force of pointed professional arguments, and the commercial consequence of the position. Do not replace forceful reasoning with generic language. Where the user makes a pointed argument, refine it — do not neutralise it.\n\nWrite in UK English throughout. Spellings: -ise not -ize, -our not -or, -re not -er.`,
      conversational: `Write as Itzik would write on his best day: warm, direct, human, conversationally professional.\n\nThe recipient should feel they are hearing from a real person — not receiving a formal letter from a firm. Keep sentences shorter. Use plain, clear language without sacrificing precision. Avoid stiff or legalistic phrasing where a natural alternative exists.\n\nWhere the email is making a case or presenting options, make the case clearly — show why the option is in the recipient's interest.\n\nALWAYS write in UK English. Spellings: -ise not -ize, -our not -or, -re not -er.`,
      warm: `Write as Itzik would write on his best day: empathetic, human, approachable, and professionally kind.\n\nAcknowledge the recipient's situation where appropriate. Lead with understanding before leading with position. Use plain, warm language. Avoid anything that could feel adversarial, cold or transactional — even where the professional position is firm.\n\nThe recipient should feel respected and heard, even where the answer is not what they wanted.\n\nALWAYS write in UK English. Spellings: -ise not -ize, -our not -or, -re not -er.`,
    };
    const personalityInstruction = PERSONALITY_INSTRUCTIONS[noraPersonality] || PERSONALITY_INSTRUCTIONS.professional;

...
export async function draftFromClaims(claims, projectMeta, apiKey, modelMode, rawNotes) {
  ...
  const userPrompt = 'YOU ARE READING RAW VOICE DICTATION FROM A PARTY WALL SURVEYOR.\n' +
    ...
    'Read the complete transcript as primary evidence and follow the full inspection sequence, including room transitions, later additions, amendments and corrections. Preserve every measurement, unit, axis, reference point, direction, location, material and stated uncertainty. Never substitute one measurement axis or reference point for another.\n' +
    'USE ALREADY-RESOLVED CONTEXT — DO NOT RE-FLAG WHAT IS ALREADY KNOWN: each claim in the checklist below carries a "section=" value, worked out live during the inspection with full context available at the time (the surveyor\'s own room declarations and everything said before it). That resolved section is authoritative — use it directly. Only mark something [UNCLEAR: concise explanation — please confirm] when the checklist itself has no section for that claim, or when the actual observation content (not its room) is genuinely missing or contradictory. Never re-flag which room something belongs to as unclear when the checklist already states it — that is already-known context, not a guess.\n' +
    'A ROOM MENTIONED IS NOT THE SAME AS A ROOM ENTERED: a note describing the stairwell or landing may reference a nearby room by name purely as a spatial reference point (e.g. "the wall abutting the bathroom", "toward the kitchen") without the surveyor having actually moved into that room. Only treat it as an actual section change when the surveyor explicitly declares moving there (e.g. "moving to the front bedroom", "in the kitchen now") — a mentioned room name inside an otherwise-continuing observation stays in its already-resolved section= from the checklist. Never invent a new section (e.g. "Hallway") that the surveyor never actually named. Never split the same section into two separate entries with the same title — every claim sharing one section= value belongs in one single section in the output, grouped together, not scattered across multiple entries.\n' +

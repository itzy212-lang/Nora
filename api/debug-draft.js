// api/debug-draft.js
//
// TEMPORARY, diagnostic-only endpoint for Phase D3 acceptance
// verification (2026-09-19) — invokes draft() against a session using
// the live production model and returns its output as JSON. Strictly
// read-only: draft() calls reconcile() (D2, read-only) and reads
// user_brain_v2 - nothing here writes to any table.
//
// Same pattern as the removed debug-reconcile.js: GET only,
// query-param secret (this endpoint is called by a tool with no
// custom-header support), 404 on any mismatch. Flagged for removal
// once D3 acceptance is confirmed - not a permanent feature.
...
const DEBUG_KEY = 'd3acc-9m2p4-draft-verify-2026';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ ...result, _debug_user_brain_used: userBrain });
  } catch (err) {
